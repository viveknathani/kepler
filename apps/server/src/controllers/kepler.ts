...
import {
  removeSchedule,
  syncSchedule,
} from '../queues/workers/workflowSchedule';
...
export async function bootstrap(c: Context<AppEnv>) {
  return sendResponse(c, 200, {
    status: 'success',
    data: await service.bootstrap(c.var.user.id),
  });
}
...
export async function getProfiles(c: Context<AppEnv>) {
  return sendResponse(c, 200, {
    status: 'success',
    data: await service.listProfiles(c.var.user.id),
  });
}
...
export async function getWorkflows(c: Context<AppEnv>) {
  return sendResponse(c, 200, {
    status: 'success',
    data: await service.listWorkflows(c.var.user.id),
  });
}
...
const scheduleInput = z.object({
  cronExpression: z
    .string()
    .trim()
    .regex(/^(\S+\s+){4}\S+$/, 'use a five-field cron expression'),
...
export async function getSchedules(c: Context<AppEnv>) {
  return sendResponse(c, 200, {
    status: 'success',
    data: await service.listSchedules(c.var.user.id),
  });
}
...
export async function getRuns(c: Context<AppEnv>) {
  return sendResponse(c, 200, {
    status: 'success',
    data: await service.listRuns(c.var.user.id),
  });
}

export async function getAgents(c: Context<AppEnv>) {
  return sendResponse(c, 200, {
    status: 'success',
    data: await service.listAgents(c.var.user.id),
  });
}

export async function getAgentRuns(c: Context<AppEnv>) {
  ...
  try {
    return sendResponse(c, 200, {
      status: 'success',
      data: await service.getAgentRuns(c.var.user.id, slug),
    });
  } catch {
    return sendResponse(c, 404, {
      status: 'error',
...
export async function getReports(c: Context<AppEnv>) {
  return sendResponse(c, 200, {
    status: 'success',
    data: await service.listReports(c.var.user.id),
  });
}
...
export async function getReport(c: Context<AppEnv>) {
  ...
  try {
    return sendResponse(c, 200, {
      status: 'success',
      data: await service.getReport(c.var.user.id, id),
    });
  } catch {
    return sendResponse(c, 404, {
      status: 'error',
...
export async function createFeedback(c: Context<AppEnv>) {
  ...
  if (!reportItemId)
    return sendResponse(c, 400, {
      status: 'error',
      message: 'report item id is required',
    });
  const parsed = z
    .object({
      action: z.enum([
  ...
  try {
    const data = await service.saveFeedback(
      c.var.user.id,
      reportItemId,
      parsed.data.action,
    );
