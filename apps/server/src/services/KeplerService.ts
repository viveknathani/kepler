...
const agents = [
  ['github-scanner', 'Search GitHub for relevant repositories, issues, and pull requests.'],
  ['paper-scanner', 'Search for relevant papers and technical writing.'],
...
export class KeplerService {
    ...
    if (!profile) {
      [profile] = await this.state.database
        ...
        .values({
          ...
          description: 'A starter profile. Edit it to teach Kepler what deserves your attention.',
    ...
    if (!workflow) {
      [workflow] = await this.state.database
        ...
        .values({
          ...
          status: 'active',
          configuration: { reportSize: 6, contentMix: { read: 0.5, build: 0.5 } },
        })
        .returning();
  ...
  listProfiles(userId: string) {
    return this.state.database.select().from(profiles).where(eq(profiles.userId, userId));
  }
  ...
  listWorkflows(userId: string) {
    return this.state.database.select().from(workflows).where(eq(workflows.userId, userId));
  }
  ...
  async createRunForSchedule(scheduleId: string) {
    ...
    const run = await this.createRun(row.workflow.userId, row.workflow.id, 'scheduled');
  ...
  async deleteSchedule(userId: string, scheduleId: string) {
    const [owned] = await this.state.database
      ...
      .limit(1);
    if (!owned) throw new Error('schedule not found');
    await this.state.database.delete(schedules).where(eq(schedules.id, scheduleId));
  }
  ...
  async setRunJobId(runId: string, jobId: string) {
    await this.state.database.update(workflowRuns).set({ jobId }).where(eq(workflowRuns.id, runId));
  }
  ...
  async executeMockWorkflow(workflowRunId: string) {
    ...
    await this.state.database.update(workflowRuns).set({
      status: 'running',
    ...
    const steps = await this.state.database
      ...
      .from(workflowSteps)
      .innerJoin(agentDefinitions, eq(workflowSteps.agentDefinitionId, agentDefinitions.id))
    ...
    for (const { step, definition } of steps) {
      const [existing] = await this.state.database
        .select()
        .from(agentRuns)
        .where(and(eq(agentRuns.workflowRunId, run.id), eq(agentRuns.workflowStepId, step.id)))
        .limit(1);
      ...
      if (!existing) {
        ...
        await this.state.database.update(agentRuns).set({
          status: 'running',
      ...
      if (definition.slug === 'github-scanner') {
        ...
        await Bun.sleep(120);
        await this.persistRunFinding(run, agentRunId, {
          title: 'A practical paper on learned query optimization',
          canonicalUrl: 'https://example.org/papers/learned-query-optimization',
          sourceType: 'paper',
          contentKind: 'read',
          summary: 'A mocked paper result; paper-scanner is the next external source to implement.',
          score: 0.72,
        });
        output = { summary: 'paper-scanner completed its mock pass.', findingCount: 1 };
      } else {
        await Bun.sleep(120);
    ...
    const [report] = await this.state.database.insert(reports).values({
      id: newId('report'),
      userId: run.userId,
      workflowRunId: run.id,
      title: 'Research radar',
      summary: 'Two promising directions selected from this run.',
      status: 'published',
    ...
    const existingItems = await this.state.database.select().from(reportItems).where(eq(reportItems.reportId, report!.id));
    if (!existingItems.length) {
      await this.state.database.insert(reportItems).values(savedFindings.slice(0, 8).map(({ finding, stageData }, index) => ({
        id: newId('ritem'),
        reportId: report!.id,
        findingId: finding.id,
        position: index + 1,
        ...
        nextSteps: finding.contentKind === 'build'
          ? ['Read the issue discussion', 'Inspect the surrounding code path']
        ...
        scores: {
          ...
          quality: (stageData as { scannerScore?: number } | null)?.scannerScore ?? 0.7,
        },
      })));
    }
